...
// util
IntervalVec<x10::lang::String *> *getIVFromBox(const rp::Box& box) {
...
// util
void setIVIntoBox(IntervalVec<x10::lang::String *>& iv, rp::Box& box) {
    rp::Scope::const_iterator it = box.scope()->begin();
...
// util
void setBoxIntoIV(rp::Box& box, IntervalVec<x10::lang::String *>& iv) {
    rp::Scope::const_iterator it = box.scope()->begin();
    for (int i = 0; it != box.scope()->end(); ++it, ++i) {
        Interval si = IntervalVec<x10::lang::String *>::getOrThrow(&iv, x10::lang::String::_make((*it)->name().c_str(), true));
        //rp::Interval i(box.get_interval(**it));
